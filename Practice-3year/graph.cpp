...
vector<vector<edge>>* read_edges(istream_iterator<int16_t>& iter) {
	...
	for (int i = 0; i < count_edges; ++i) {
		...
		_edge.second = *(iter++);
		size = edges->size();
		if ((size < _edge.first) || (size < _edge.second)) {       //добавляем для новых вершин списки рёбер
...
/*Метод ветвей и границ может закончиться и вернуть решение в недостроенном виде;
достроим его тем же способом, который использутся при построении допустимого решение в lower_bound*/
